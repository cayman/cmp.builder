...
exports.init = function (grunt) {
    ...
    cmpUtil.getSimpleId = function (bower) {
        ...
        if (!bower.hashDir) {
            if(!bower.version){
                grunt.fail.fatal('bower.json file mast have version or hashDir field');
            }
    ...
    var CmpLink = {
        _constructor: function (type, file, version, rel) {
            this.type =type;
        ...
        constructorJs: function (file, version) {
            return this._constructor('text/javascript', file, version, null);
        },
        constructorHtml: function (file, version, rel) {
            return this._constructor('text/html', file, version, rel);
        },
        constructorCss: function (file, version, rel) {
            return this._constructor('text/css', file, version, rel);
        },
        constructorIcon: function (file, version, rel) {
            return this._constructor('image/x-icon', file, version, rel);
        },
        ...
        log: function(){
            return  'type='+this.type + ', ' +
                ...
                (this.media ? ('media='+this.media+ ', '):'') + this.src;
        }
    };
    ...
    CmpProto.getScripts = function (basePath, pathField, fileFields, minJs) {
        ...
        //    grunt.log.writeln( '  ', script.log());
        //});

        scripts.flat = function(field) {
            var field = 'path';
            for (var i = 0; i < this.length; i++) {
                grunt.log.writeln( '  ', this[i][field]);
                if(this[i][field]){
                    this[i] = this[i][field];
                }
            }
    ...
    CmpProto.getLinks = function (basePath, pathField, fileFields) {
        ...
        this._parseMain(pathField, fileFields,function(cmpObject, pathValue, fileValue){
            ...
            switch (path.extname(pathname)){
                case '.html':
                case '.htm':
                    links.push(Object.create(CmpLink).constructorHtml(pathname,cmpObject.version,'import'));
                    break;
                case '.css':
                    links.push(Object.create(CmpLink).constructorCss(pathname,cmpObject.version,'stylesheet')
                        .params(parsed.query));
                    break;
                case '.ico':
                case '.icon':
                    links.unshift(Object.create(CmpLink).constructorIcon(pathname,cmpObject.version,'icon'));
                    links.unshift(Object.create(CmpLink).constructorIcon(pathname,cmpObject.version,'shortcut icon'));
                    break;
            }
        },true);
        grunt.verbose.writeln('>>'.cyan + 'links = ', links);
        //links.map(function(link){
        //    grunt.log.writeln( '  ', link.log());
        //});

        links.flat = function(field) {
            var field = 'path';
            for (var i = 0; i < this.length; i++) {
                grunt.log.writeln( '  ', this[i][field]);
                if(this[i][field]){
